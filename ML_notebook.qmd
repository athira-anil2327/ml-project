...
We will begin by analyzing the dataset to understand its structure and characteristics. This includes visualizing distributions, checking for missing values, and examining class balance.
...
**dentifying Duplicate Rows:** The code `iris_df[iris_df.duplicated()]` filters and displays the duplicate rows in the `iris_df` DataFrame, allowing us to inspect and address any redundancy in the dataset by showing which rows are duplicated.
```{python}
...
if 'variety' in iris_df.columns:
    ...
    iris_num = iris_df.drop('variety', axis=1)
      # Use drop to remove the column and keep the rest
